...
Q2: 深度融合台显示 "暂无题材数据" 或 "N/A"
    ...
    解决：
        ...
        检查 stock_data_provider.py 是否为 V57.0 版本（支持混合接口）。

Q3: 输入股票代码时界面卡顿

    解决：这是 Streamlit 的特性。请确保一次性输入完整的 6 位代码并回车。V56.0 版本已加入防抖逻辑，只有输入满 6 位数字时才会触发刷新。
