...
本项目使用 `efinance` 和 `akshare` 作为行情数据源。为提高稳定性，已经将两者封装为一个统一的数据提供模块 `src/stock_data_provider.py`，主要特点：

- 内置重试（默认 3 次）与指数退避（exponential backoff），减少因网络抖动导致的失败。
- 对 efinance/akshare 的返回做列名与时间戳标准化，兼容不同接口返回格式。
- 优先使用配置的来源（efinance 或 akshare），失败时自动回退其他来源。
- 临时禁用系统代理（若您希望给 AI 投喂数据使用代理，则不会影响行情请求）。
- 简单的 circuit breaker（故障计数）用于在某个源连续失败后短时停用以避免被封禁。

如何使用：项目中 `data_layer.DataLayer` 已经使用 `StockDataProvider` 并对外提供统一的 `get_kline/get_realtime_info/get_stock_news` 等方法。

如果仍然遇到数据拉取不稳定的情况，请：

1. 检查本地网络和目标接口访问是否受限（防火墙/公司代理等）。
2. 启用日志（将 `logging` level 调为 DEBUG）查看失败原因与错误堆栈。
3. 若需要更大并发或更低延迟，请考虑使用付费或官方数据接口（例如 Wind/同花顺）代替爬虫型接口。
