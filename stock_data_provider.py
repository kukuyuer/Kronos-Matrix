...
class StockDataProvider:
    ...
    def _get_secucode(code):
        """格式: 000001.SZ (用于 DataCenter 财务接口)"""
    ...
    def _get_market_symbol(code):
        """格式: SZ000001 (用于 PageAjax F10文本接口)"""
        code = str(code).zfill(6)
    ...
    def get_financial_indicators(stock_code, use_proxy=True):
        ...
        if data:
            ...
            res["roe"] = latest.get('ROEWEIGHTED', '-') # 加权ROE
            res["gross_margin"] = latest.get('XSMLL', '-') # 销售毛利率
            res["net_margin"] = latest.get('XSJLL', '-')   # 销售净利率
            res["profit_yoy"] = latest.get('GSJLR_TBZZ', '-') # 归属净利润同比
            res["revenue_yoy"] = latest.get('YYSR_TBZZ', '-') # 营收同比
    ...
    def get_core_concepts(stock_code, use_proxy=True):
        """
        [修正] 获取核心题材与主营 (使用 emweb PageAjax 接口)
        使用 SZ001309 格式
        """
            ...
            if resp:
                ...
                if 'hxtc' in data and data['hxtc']:
                    ...
                    for item in data['hxtc']:
                        name = item.get('hxtcname', '')
                        content = item.get('hxtccontent', '')
                        if name:
                            concepts.append(f"【{name}】 {content[:40]}...")
                    info["concepts"] = concepts
                    if concepts: info["lead_concept"] = concepts[0].split('】')[0].replace('【','')
                
                # 尝试从 Core 接口取业务摘要
                if 'ywfx' in data and data['ywfx']:
                    zygc_list = [x.get('zygc','') for x in data['ywfx'] if x.get('zygc')]
                    if zygc_list:
                        info["business"] = "，".join(zygc_list[:5])
        except: pass

        # 2. 详细主营 (BusinessAnalysis) - 备用
                ...
                if resp:
                    ...
                    if 'zygc' in data and data['zygc']:
                        ...
                        if isinstance(latest, list):
                            prods = [p.get('ITEM_NAME','') for p in latest if p.get('ITEM_NAME')]
                            if prods: info["business"] = "，".join(prods[:6])
            except: pass
